...
/**
 * All cards of one book in the exact browse order for [sortMode], shared by
 * the browse screen and the widget so both always agree on the sequence.
 *
 ...
 */
suspend fun CardRepository.cardsInBrowseOrder(
    bookId: Long,
    sortMode: SortMode
): List<Card> {
    val all = getCards(bookId, sortMode)
    if (sortMode != SortMode.DIRECTORY) return all
    val orders = getCategoryOrders(bookId).associate { it.path to it.position }
