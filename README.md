...
| 模块 | 能力 |
|---|---|
| **闪卡浏览** | HorizontalPager 左右滑动；浏览顺序固定为**目录顺序（与目录页一致）**（标题序/导入序仅保留后端实现，未暴露 UI）；重启恢复上次位置；**与微件进度实时双向同步**（微件翻卡后回前台自动对齐）；顶部完整路径（超宽才省略）；"第 x / 共 y 张" |
...
| **目录** | 多级树展开/收起（**M3 动效**：箭头旋转 90°、子行下滑淡入、兄弟行平滑让位、收起快速淡出）；节点 **⋮ 菜单**：同级上移/下移（顺序持久化）、重命名（整组卡片路径批量迁移）、删除（卡片移入未分类或删除）；最低级目录也可展开查看卡片列表；右上角**放大镜检索**（目录与卡片全库搜索，命中高亮并自动展开定位） |
| **编辑** | 卡片右上角编辑按钮；改标题/正文/分类（分类选择器，含「新建分类…」输入框） |
| **设置** | 主题（跟随系统/浅色/深色）、**强调色**（动态取色/绿/蓝/橙/紫/莫兰迪绿/莫兰迪蓝/莫兰迪陶土）、**正文字号 + 微件字号两个独立滑块**、导入/导出入口；主题/强调色/字号变更**立即推送到微件**；界面配色体系（底部导航栏、各页标题栏、页面背景）随主题/强调色整体变化 |
| **桌面微件** | Glance；半透明亚克力圆角面板；路径栏（点击展开/收起完整路径）、标题、正文（LazyColumn 可滚动，**独立字号设置**）、上一张/下一张按钮（控件颜色跟随强调色）；与 App 共享进度与排序（DataStore，共用 `cardsInBrowseOrder` 单一排序实现），App 内翻页/设置变更即联动更新 |
...
│   ├── CardOrdering.kt     （cardsInBrowseOrder：App 与微件共用的浏览排序实现）
